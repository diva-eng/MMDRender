...
void cPMDIK::initialize( const PMD_IK *pPMDIKData, cPMDBone pBoneArray[] )
{
	...
	for( unsigned char i = 0 ; i < m_cbNumLink ; i++ )
	{
		...
		if([[NSString stringWithCString:m_ppBoneList[i]->getName() encoding:NSShiftJISStringEncoding] isEqualToString:@"左ひざ"]
		   || [[NSString stringWithCString:m_ppBoneList[i]->getName() encoding:NSShiftJISStringEncoding] isEqualToString:@"右ひざ"]) 
		{
