...
void cBulletPhysics::update( float fElapsedFlame )
{
	...
	// p_g_
	float fMilliSec = fElapsedFlame * (1000.0f / 30.0f);
	float fixedTimeStep= (1000.0f/60.f);
	int nMaxSubStep= (fMilliSec / fixedTimeStep) + 1;
	m_pBtWorld->stepSimulation( fMilliSec, nMaxSubStep );
}
...
#pragma mark ---
bool cBulletPhysics::isInitialized( void )
{
